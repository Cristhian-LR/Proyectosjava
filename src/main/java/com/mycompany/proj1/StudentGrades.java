...
public class StudentGrades {
    ...
    public int calculatePartialGrade(int examScore, int homeworkScore) {
        double firstPartialGrade = (examScore * 0.7) + (homeworkScore * 0.3);//calcula la calificacion parcial
        return (int) (firstPartialGrade);//se convierte a entero antes de devolver el resultado
    }

    public int calculateFinalGrade(int firstPartial, int secondPartial, int thirdPartial, int finalExam) {//recibe 3 parametros para obtener la calificacion final
        double averagePartials = (firstPartial + secondPartial + thirdPartial) / 3.0;
        double finalGrade = (averagePartials * 0.5) + (finalExam * 0.5);
    ...
    public String checkFailureByAbsences(int totalHours, int absences) {
        ...
        if (absencePercentage > 10) {//calcula si el numero de faltas es mayor al 10%
